...
import { Game } from '../models/Game.js'; // 1. Importa el Modelo
...
const jsonPath = path.join(__dirname, '../jsons/Games.json');
...
const seedDB = async () => {
  try {
    ...
    const gamesData = fs.readFileSync(jsonPath, 'utf-8');
    const games = JSON.parse(gamesData); // Convierte el string JSON a un objeto

    // 7. --- Inserta los nuevos datos ---
    console.log('Insertando datos...');
    await Game.insertMany(games); // insertMany es la mejor opción
